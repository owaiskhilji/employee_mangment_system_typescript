...
async function removeEmployee() {
    ...
    if (data_reup.re_up === 'delete') {
        ...
        let index = data.findIndex((e) => e.emplid === removedata.emplid2);
            ...
            let update_id = await inquirer.prompt([
                {
                    name: 'findindexnum',
                    type: 'number',
                    message: 'Which index number do you want to change the employee s ID?'
                },
            ]);
            let get_ID = data.findIndex((e) => e.emplid === update_id.findindexnum);
            if (get_ID !== -1) {
            ...
            let update_name = await inquirer.prompt([
                {
                    name: 'findindexnum',
                    type: 'number',
                    message: 'Which index number do you want to change the employee s name?'
                },
            ]);
            let get_name = data.findIndex((e) => e.emplid === update_name.findindexnum);
            if (get_name !== -1) {
            ...
            let update_salary = await inquirer.prompt([
                {
                    name: 'findindexnum',
                    type: 'number',
                    message: 'Which index number do you want to change the employee s salary ?'
                },
            ]);
            let get_salary = data.findIndex((e) => e.emplid === update_salary.findindexnum);
            if (get_salary !== -1) {
